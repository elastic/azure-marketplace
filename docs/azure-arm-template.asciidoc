...
* X-Pack Security is enabled, and the {xpackdocs}/built-in-roles.html[built-in roles] `elastic`, `kibana` and
...
`vmSizeKibana`::
The {vms}[Azure VM SKU] to use for data nodes. Different VM SKUs have different CPU, RAM,
temporary storage space and network bandwidth. The Kibana VM always uses standard storage for the OS disk. The default value is `Standard_A2`.
...
Kibana communicates with Elasticsearch through the <<internal-loadbalancer, internal load balancer>>
...
X-Pack features, including Security, for a trial period of 30 days.
...
[NOTE]
--
The template does **not** currently deploy Logstash. The `logstash_system` built-in user account
must still be configured however, and can be used to communicate with Elasticsearch
if Logstash is also deployed.
--
...
To integrate SAML SSO, you

TODO! Requirements

TODO! Indicate that Basic Authentication realm is also configured
...
if setting up a `CNAME` of `saml-aad` on the `elastic.co` domain to point to Kibana, the value will be `https://saml-aad.elastic.co:5601` for Identifier (EntityID).
Additionally, the Reply URL input should be `https://saml-aad.elastic.co:5601/api/security/v1/saml`. If you will not be configuring
...
When an Enterprise application is first created, an https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-application-manifest[Application Manifest] is also created that controls the application's identity configuration with respect to AAD. Amongst other things, the app manifest specifies which roles can be declared for the Enterprise application, in the `appRoles` array.

To view the `appRoles`, navigate to the `App registrations` menu item within the Azure Active Directory blade, select the Enterprise application in question, and click the Manifest button
...
In addition to these parameters, the following conditions must be satisfied:

* `esVersion` **must** be one that supports the SAML realm i.e. Elasticsearch 6.2.0 or later
* `xpackPlugins` **must** be `Yes` to install a trial license of X-Pack platinum features
...
Since SAML SSO integration also configures Bacis Authentication access, role mappings
...
[source,sh]
----
curl -XPUT "https://localhost:9200/_xpack/security/role_mapping/saml-kibana-user" -H 'Content-Type: application/json' -u elastic -d'
{
  "roles": [ "kibana_user" ],
  "enabled": true,
  "rules": {
...
[source,sh]
----
curl -XPUT "https://localhost:9200/_xpack/security/role_mapping/saml-superuser" -H 'Content-Type: application/json' -u elastic -d'
{
  "roles": [ "superuser" ],
  "enabled": true,
  "rules": {
...
When using Application Gateway for the `loadBalancerType`, you can **only** use `esHttpCertBlob` parameter (and optional `esHttpCertPassword`). SSL/TLS to Application Gateway is secured using `appGatewayCertBlob` parameter as described in <<application-gateway, Application Gateway section>> and Application Gateway performs
...
The certificate can be extracted from the PKCS#12 archive using openssl
...
You can secure communication between the browser and Kibana with TLS wiht the following parameters
...
. https://linux.die.net/man/1/base64[base64] on Linux, or https://www.openssl.org/docs/man1.0.2/apps/openssl.html[openssl] on Linux and MacOS
+
[source,sh]
.Using base64
----
...
[NOTE]
--
PEM certificates are already base 64 encoded. Irrespective to this, you should still base 64 encode PEM certificates; _all certificates and PKCS#12 archives_ are expected to be base 64 encoded, to keep template usage simple i.e. if a parameter contains a certificate or archive, base 64 encode it.
--
...
can be downloaded from a public URI at deployment and installation time.

=== X-Pack

A trial license for the Platinum features of X-Pack can be deployed for Elasticsearch, and Kibana, if also deployed, using the following parameter

`xPackPlugins`::
Select `Yes` to install a trial license of the platinum features. A trial license provides access to platinum features for 30 days. a value of `No`
+
. for Elasticsearch and Kibana 6.3.0+ will deploy with a free perpetual license to the basic features. These versions use a distribution with X-Pack included, and the license applied determines which features are activated for use.
. for Elasticsearch and Kibana prior to 6.3.0, the X-Pack plugin will not be installed when a value of `No` is passed. These versions
...
be found within the Azure portal:

TODO! image here

The same information can be retrieved using the Azure CLI tools
